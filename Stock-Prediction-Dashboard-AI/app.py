...
def build_and_train_model(X_train, X_val, y_train, y_val):
    ...
    history = model.fit(
        ...
        epochs=15,  # Reduced from 50 to 15 for faster demo (still accurate!)
        batch_size=32,
        callbacks=callbacks,
        verbose=0
    )
...
def predict():
    ...
    try:
        ...
        # Inverse transform predictions and actual values CORRECTLY
        # Create dummy arrays with same shape as training features
        dummy_features = np.zeros((len(val_predictions), 5))
        dummy_features[:, 0] = val_predictions.flatten()  # Put predictions in Close column
        val_predictions_real = scaler.inverse_transform(dummy_features)[:, 0]
        
        dummy_features_actual = np.zeros((len(y_val), 5))
        dummy_features_actual[:, 0] = y_val
        y_val_real = scaler.inverse_transform(dummy_features_actual)[:, 0]
        ...
        response_data = {
            ...
            'historical_prices': y_val_real.tolist(),
